...
class Perceptron(RedNeuronal):
    ...
    def __init__(self, alpha, umbral, tamentrada, tamsalida, max_epocas):
...
if __name__ == "__main__":
    ...
    p = Perceptron(0.3,0,modo.X_train.shape[1], modo.y_train.shape[1],100)
    p.train(modo.X_train, modo.y_train)

    predicciones = p.predict(modo.X_test, modo.y_test.shape[1])

    print("Porcentaje de error al clasificar los datos: " + str(p.test(predicciones,modo.y_test)))
